...
const TOOL_LIST = [
  { name: 'worktracker_list_items', description: 'List work items.', inputSchema: { type: 'object', properties: {
  ...
  { name: 'worktracker_get_item', description: 'Get one work item with its events.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'worktracker_create_item', description: 'Create a new work item.', inputSchema: { type: 'object', properties: {
  ...
  { name: 'worktracker_update_item', description: 'Patch fields on a work item.', inputSchema: { type: 'object', properties: {
    id: { type: 'string' }, patch: { type: 'object' }, expected_version: { type: 'number' },
  }, required: ['id', 'patch', 'expected_version'] } },
  { name: 'worktracker_transition', description: 'Move a work item to a new status.', inputSchema: { type: 'object', properties: {
  ...
  { name: 'worktracker_comment', description: 'Append a comment to a work item.', inputSchema: { type: 'object', properties: {
    id: { type: 'string' }, body: { type: 'string' }, expected_version: { type: 'number' },
  }, required: ['id', 'body'] } },
  { name: 'worktracker_link_items', description: 'Link two work items.', inputSchema: { type: 'object', properties: {
  ...
  { name: 'worktracker_list_boards', description: 'List all kanban boards.', inputSchema: { type: 'object', properties: {} } },
  { name: 'worktracker_get_board', description: 'Get one board by id.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'worktracker_create_board', description: 'Create a new board. Admin only.', inputSchema: { type: 'object', properties: {
    name: { type: 'string' }, description: { type: 'string' },
    kinds: { type: 'array', items: { type: 'string' } },
    columns: { type: 'array' }, is_default: { type: 'boolean' },
  }, required: ['name', 'columns'] } },
  { name: 'worktracker_update_board', description: 'Patch a board. Admin only.', inputSchema: { type: 'object', properties: {
    id: { type: 'string' }, name: { type: 'string' }, description: { type: 'string' },
    kinds: { type: 'array', items: { type: 'string' } },
    columns: { type: 'array' }, is_default: { type: 'boolean' },
  }, required: ['id'] } },
  { name: 'worktracker_delete_board', description: 'Delete a board. Admin only.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'worktracker_enrich', description: 'Run Grill/Wayfind on a work item.', inputSchema: { type: 'object', properties: {
    id: { type: 'string' }, stage: { type: 'string', enum: ['grill', 'wayfind', 'both'] }, enricher: { type: 'string' },
  }, required: ['id', 'stage'] } },
  { name: 'worktracker_dispatch', description: 'High-level ship-it tool.', inputSchema: { type: 'object', properties: {
    id: { type: 'string' }, options: { type: 'object' },
  }, required: ['id'] } },
  { name: 'worktracker_set_reminder', description: 'Set a reminder (v0.5 stub).', inputSchema: { type: 'object', properties: {
  ...
  try {
    switch (req.method) {
      ...
      case 'notifications/initialized':
        return v2Result(req, {});
      case 'tools/list':
        return v2Result(req, { tools: TOOL_LIST });
      case 'tools/call': {
